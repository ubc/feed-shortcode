...
class CTLT_Feed_Shortcode {
	...
	public function init() {
		add_shortcode( 'feed', array(__CLASS__, 'feed_shortcode' ) );
	}
		...
		?>
			...
			<?php if ($excerpt):  ?>
				...
				if(is_numeric($excerpt_length) && $excerpt_length >0 )
					...
					echo clf_base_strshorten($item->get_description(),$excerpt_length);
				?></div>
			...
			foreach ( (array) $rss_items as $item ): 
				...
				if ( $show_author ) {
					...
					if ( $author) {
						...
						$author = ' <cite class="author">' . esc_html( strip_tags( $author ) ) . '</cite>';
					}
				}// End display post item's author
				...
				}//End: YC Oct 2012 - sort by and show posts' last modify date instead of publish date
			
			?>
			
			<li class="<?php echo $odd_or_even; ?>"><a class="rsstitle" href="<?php echo esc_url( $item->get_permalink() ); ?>" <?php echo $target; ?> ><?php echo esc_html($item->get_title());?></a>
			<!-- YC, Oct 2012 - show the posts' author and last updated date-->
			<?php echo $author;  ?> <?php if ($updated_date){echo '<span class="rss-bracket">(</span> <span class="rss-date">' . $updated_date . '</span><span class="rss-bracket">)</span>';}  ?>
